...
async def get_analytics():
    ...
    users = []  # this will be a list of dict, having strings will be a problme
    ...
    stats = {
        ...
        "avg_item_name_length": (
            float(item_name_lengths.mean()) if item_name_lengths.size > 0 else 0.0
        ),
        "avg_user_username_length": (
            float(user_username_lengths.mean())
            if user_username_lengths.size > 0
            else 0.0
        ),
        "max_item_name_length": (
            int(item_name_lengths.max()) if item_name_lengths.size > 0 else 0
        ),
        "max_user_username_length": (
            int(user_username_lengths.max()) if user_username_lengths.size > 0 else 0
        ),
    }
    ...
    # fixed endpoint to return image
    return JSONResponse({
    "stats": stats,
